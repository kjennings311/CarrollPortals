...
namespace Carroll.Portals.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> ExportClaim(string Claim, char Type)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //Sending request to find web api REST service resource GetAllEmployees using HttpClient  
                //   HttpResponseMessage Res = await client.GetAsync("api/data/GetEmployeeLeaseRider?riderid="+id);
                HttpResponseMessage Res = await client.GetAsync("api/data/GetExportClaim?Claim=" + Claim + "&Type=" + Type);
                //Checking the response is successful or not which is sent using HttpClient  
                string Propid = "";
                ...
                if (Res.IsSuccessStatusCode)
                {
                    //Storing the response details recieved from web api   
                    var EmpResponse = Res.Content.ReadAsStringAsync().Result;
                    var propres = JsonConvert.DeserializeObject<ExportClaim>(EmpResponse);
